...
?>
<!-- Sidebar -->
<ul class="navbar-nav bg-gradient-primary sidebar sidebar-light border-right border-grey shadow-lg accordion " id="accordionSidebar">
        ...
        <?php if ($m_satuan['id_menu'] == 1) : ?>
            <li class="nav-item <?php echo $act; ?>">
                <a class="nav-link " href="<?php echo site_url($m_satuan['url']); ?>">
                    ...
                    <span class="text-capitalize text-warning"><?php echo $m_satuan['menu']; ?>
                    </span>
                </a>
            </li>
        ...
        <?php else : ?>
            <?php if ($m_satuan['is_parent'] == 1) : ?>
                <li class="nav-item <?php echo $act; ?> mb-2">
                    <a class="nav-link <?php echo $aria; ?> py-2" href="javascript:void(0);" data-toggle="collapse" data-target="#collapse<?php echo $m_satuan['id_menu']; ?>" aria-expanded="true" aria-controls="collapse<?php echo $m_satuan['id_menu']; ?>">

                        <i class="fa-fw <?php echo $m_satuan['icon']; ?> text-white"></i>
                        <span class="text-capitalize text-white"><?php echo $m_satuan['menu']; ?></span>
                    </a>

                    <div id="collapse<?php echo $m_satuan['id_menu']; ?>" class="collapse <?php echo $tog; ?>" aria-labelledby="heading<?php echo $m_satuan['id_menu']; ?>" data-parent="#accordionSidebar">
                            ...
                            <?php foreach ($menu as $m_coll) : ?>
                                <?php if ($m_coll['menu_parent'] == $m_satuan['id_menu']) : ?>
                                    <?php if ($this->uri->segment(1) . '/' . $this->uri->segment(2) == $m_coll['url']) : ?>
                                        <a class="collapse-item text-primary font-weight-bold bg-gray-200 py-2" href="<?php echo site_url($m_coll['url']); ?>"><i class="fas fa-fw fa-check-square text-primary"></i> <?php echo ucwords($m_coll['menu']); ?></a>
                                    <?php else : ?>

                                        <a class="collapse-item text-secondary font-weight-bold  py-2" href="<?php echo site_url($m_coll['url']); ?>"><i class="fas fa-fw fa-minus-square text-gray-400"></i> <?php echo ucwords($m_coll['menu']); ?></a>
                                    <?php endif; ?>
