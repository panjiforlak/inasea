...
<div class="col-12">
    <div class="card recent-sales overflow-auto">
        <div class="filter">
            ...
            <a href="<?php echo site_url(); ?>master/user" class="btn btn-sm btn-danger add" style="font-size: 12px;margin-right:20px;">
        ...
        <div class="card-body">
            ...
            <form method="POST" action="<?php echo site_url('master/user_form_add/add'); ?>">

                <div class="row mb-3">
                    <label for="inputEmail" class="col-sm-2 col-form-label">Email <sup class="text-danger">*</sup></label>
                    <div class="col-sm-5">
                        <input type="text" class="form-control form-control-sm text-lowercase" name="email" required>
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="inputPassword" class="col-sm-2 col-form-label">Password <sup class="text-danger">*</sup></label>
                    <div class="col-sm-3">
                        <input type="password" class="form-control form-control-sm" name="password1" required placeholder="set password">
                    </div>
                    <div class="col-sm-2">
                        <input type="password" class="form-control form-control-sm" name="password2" required placeholder="confirm password">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="inputPassword" class="col-sm-2 col-form-label">Full Name <sup class="text-danger">*</sup></label>
                    <div class="col-sm-5">
                    ...
                    <div class="col-sm-5">
                        <select class="form-control form-control-sm select2" id="select_branch" name="branch_id">
                            ...
                            <?php foreach ($get_branch->result_array() as $gbranch) : ?>
                                <option value="<?php echo $gbranch['id']; ?>"><?php echo $gbranch['branch_name']; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-5">
                        <select class="form-control form-control-sm select2" name="role_id" id="select_role" required>
                            ...
                            <?php foreach ($get_role->result_array() as $grole) : ?>
                                <option value="<?php echo $grole['id_role']; ?>"><?php echo ucwords($grole['role']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                <div class="row mb-3">
                    ...
                    <div class="col-sm-5">
                        ...
                        <div class="form-check">
                            ...
                            <input class="form-check-input" type="checkbox" id="gridCheck1" value="1" name="is_active" checked>
...
<script>
    ...
    $('#select_branch').change(function() {
        var id = $(this).val();
        $.ajax({
            url: "<?php echo site_url(); ?>master/ajx_dpt/" + id,
            method: "GET",
            data: {
                id: id
            },
    ...
    $('#select_department').change(function() {
        var id = $(this).val();
        $.ajax({
            url: "<?php echo site_url(); ?>master/ajx_pst/" + id,
            method: "GET",
            data: {
                id: id
            },
            ...
            success: function(data) {
                ...
                for (i = 0; i < data.length; i++) {
                    html += '<option  value=' + data[i].id_role + ' >' + data[i].role + '</option>';
                }
