...
class Select_model extends CI_Model
{
    ...
    {
        if (!empty($param)) {
            $this->db->where('id_menu', $param);
        }
        $this->db->order_by('id_menu', 'asc');
        $this->db->where('is_delete !=', 1);
        $qry = $this->db->get('user_menu');
        return $qry;
    }
    ...
    public function get_role_access($param1 = "")
    {
        ...
        $qry = $this->db->get('user_access_menu');
        return $qry;
    }
    public function get_user($param1 = "")
    {
        ...
        $this->db->join('user_role', 'users.role_id=user_role.id_role', 'left');
    ...
    public function get_delivery_order($s_row = "", $uri, $s_mar = "", $s_pro = "", $s_sls = "")
    {
        ...
        $this->db->join('sales as b', 'a.sales_id=b.id_sales', 'left'); //first
        $this->db->join('customer as c', 'b.customer_id = c.id_customer', 'left'); //second
        $this->db->join('market as d', 'c.market_id = d.id_market', 'left'); //ext
        $this->db->join('product as e', 'a.product_id = e.id_product', 'left'); //ext
    ...
    public function get_stock($s_row = "", $uri, $s_war = "", $s_pro = "", $s_qty = "")
    {
        ...
        $this->db->join('warehouse as b', 'a.warehouse_id = b.id_warehouse', 'left'); //ext
        $this->db->join('product as c', 'a.product_id = c.id_product', 'left'); //ext
    ...
    public function get_sales_report($s_row = "", $uri, $s_fak = "", $s_cus = "", $s_from = "", $s_end = "")
    {
        ...
        $this->db->join('customer as b', 'a.customer_id = b.id_customer', 'left'); //ext
